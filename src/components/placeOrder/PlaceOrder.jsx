...
const PlaceOrder = () => {
  ...
  const addToCart = () => {
    ...
    // const isItemInCart = item.find((elem) => elem == item);
    // const isItemInCart = item.find((elem) => elem == productInfo);
    ...
    // const flag = item.map((elem) => {
    //   if (elem.id == productInfo.id) {
    //     return true;
    //   } else {
    //     false;
    //   }
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} sm={3}>
          <Paper variant="outlined" className="placeorder_order">
            ...
            <div>
              <div style={{ lineHeight: "25px" }}>{productInfo?.category}</div>
            </div>
            <button className="placeorder_button addtocart" onClick={addToCart}>
              Add to Cart
            </button>
            <Link to={"/checkout"}>
              <button className="placeorder_button buynow" onClick={addToCart}>
                Buy Now
              </button>
            </Link>
          </Paper>
